...
globals[
  ...
  ; fixing parameter see "all parameters" model version to adjust these settings
  n-identity  ;; number of political identities
  identity-colour ;; colour of agents
  with-conflict   ;; influence of conflict on happiness
  past-choices-length ;; the number of choices in a an agents choice list
  conflict-range  ;; the minimal absolute difference between moral decision weights inducing moral conflict
  similar-needed  ;; proportion of agents of the perceived same political identity to be happy
  choice-proportion  ;; the proportion of randomly selected agents who must make a choice
  density  ;; the proportion of space occupied by agents
  ticks-per-simulation  ;; number of ticks in a simulation
  n-simulations ;; number of simulations recorded
...
to setup
  ...
  set n-identity "binary"
  set identity-colour "binary"
  set with-conflict "yes"
  set past-choices-length 5
  set conflict-range 0.3
  set similar-needed 62.5
  set choice-proportion 85
  set n-simulations 20
  set ticks-per-simulation 100
...
to identify-friends                        ; lets turtle perform social inference about the political identity of their neighbouring turtles by comparing their past-choices
  ask turtles [
    ...
    if friend_count < 2 and length friend_string > 4
    [ let this_one who
      user-message word "problem! check out turtle " this_one ]
